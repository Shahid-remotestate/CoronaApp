...
export class CityComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.routes.paramMap.subscribe(data => {
      this.CountryName = data.get('country');
    });
    this.appServices.getcitydetail(this.CountryName).subscribe({
      error: err => {
        this.flag2 = false;
      },
      next: data => {
        this.city = data;
        this.city.length > 0 ? this.flag = !!this.city[0].Province : this.flag2 = false;
      }
    });
